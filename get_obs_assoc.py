...
class get_obs_assoc(Resource):
    ...
    #def __init__(self, user='root', host='localhost', pw_='', db='knowledge', table_name="ModelStorage"):
        ...
        if num_target_tables > 0:
            for obs in obs_list:
                tbl_query = f'''
                            ...
                            '''
                self.rel_db.cursor.execute(tbl_query) #add rel_db back
                ret_tables: Tuple[Tuple[str]] = self.rel_db.cursor.fetchall() #add rel_db back
                if ret_list == None:
                    ret_list = [ret_tables]
                else:
